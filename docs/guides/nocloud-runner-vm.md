...
`proxmox_qemu_vm` and `proxmox_lxc_container` also accept an optional `power` attribute as the declarative counterpart to the hooks above. `power = true` means "running": apply starts a guest that refresh observed stopped. `power = false` means "stopped": apply shuts a running guest down through the single Proxmox shutdown task, which waits `power_shutdown_timeout` seconds (default 60, range 1-600, graceful timeout then forced stop server-side). QEMU `paused` counts as powered on: `power = true` never resumes a paused guest, and `power = false` force-stops it server-side. Reconciliation happens only during apply: refresh keeps writing observed reality into state without acting, so a manually stopped guest stays stopped until the next apply - and with `power = true` that apply restarts it. That restart is the feature, not a bug; remove the `power` attribute to stop reconciling and treat the guest as maintenance-stopped. `status` and `uptime` stay observed-only in either mode. `power` conflicts with `start_on_create` (drop the hook when adopting declarative power) and coexists with `stop_on_destroy`; unset means no power management, and `power` is never inferred for imported or data source reads.
...
9. **Refresh does not restart; apply reconciles `power`**: without `power` set, run `terraform apply` again on an idle, manually stopped guest; the guest must stay stopped, and the plan must be clean or update-only. With `power = true`, the next apply must restart a manually stopped guest (that declarative reconcile is the feature), while `terraform plan`/refresh alone must never start or stop it. With `power = false` and `power_shutdown_timeout`, an apply against a running guest must shut it down gracefully and force it off after the timeout; verify the PVE task log shows the single `qmshutdown`/`vzshutdown` task with `forceStop=1`.
