...
func TestClientSnapshotMethods(t *testing.T) {
	for _, kind := range []snapshotKind{snapshotKindQEMU, snapshotKindLXC} {
		t.Run(string(kind), func(t *testing.T) {
			...
			server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
				assertTokenAuth(t, r)
				switch {
				case r.URL.Path == "/api2/json/nodes/pve-1/"+string(kind)+"/101/snapshot" && r.Method == http.MethodPost:
				...
				case r.URL.Path == "/api2/json/nodes/pve-1/"+string(kind)+"/101/snapshot" && r.Method == http.MethodGet:
					writeEnvelope(t, w, []map[string]any{
						{"name": "pre-deploy", "description": "before terraform apply", "parent": "base", "snaptime": 1700000000},
						{"name": "other", "description": "unrelated"},
					})
				case r.URL.Path == "/api2/json/nodes/pve-1/"+string(kind)+"/101/snapshot/pre-deploy/config" && r.Method == http.MethodPut:
					assertFormValues(t, r, url.Values{"description": {"updated"}})
					writeEnvelope(t, w, nil)
				case r.URL.Path == "/api2/json/nodes/pve-1/"+string(kind)+"/101/snapshot/pre-deploy" && r.Method == http.MethodDelete:
			...
			if err := client.createSnapshot(ctx, kind, "pve-1", 101, "pre-deploy", stringPtr("before terraform apply")); err != nil {
				t.Fatalf("createSnapshot() unexpected error: %v", err)
			}

			snap, err := client.getSnapshot(ctx, kind, "pve-1", 101, "pre-deploy")
			if err != nil {
			...
			if err := client.updateSnapshot(ctx, kind, "pve-1", 101, "pre-deploy", "updated"); err != nil {
				t.Fatalf("updateSnapshot() unexpected error: %v", err)
			}

			if err := client.deleteSnapshot(ctx, kind, "pve-1", 101, "pre-deploy"); err != nil {
...
func TestClientSnapshotGetNotFound(t *testing.T) {
	for _, kind := range []snapshotKind{snapshotKindQEMU, snapshotKindLXC} {
		t.Run(string(kind), func(t *testing.T) {
			ctx := context.Background()
			...
			if _, err = client.getSnapshot(ctx, kind, "pve-1", 101, "missing"); err == nil {
...
func TestParseSnapshotImportID(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		node, vmID, name, err := parseSnapshotImportID(tc.input)
		if tc.wantErr {
			if err == nil {
